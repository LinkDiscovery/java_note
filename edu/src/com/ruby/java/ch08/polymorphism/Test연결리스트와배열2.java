...
class LinkedList3 {
	...
	Node3 head; // int first 는 정수를 저장하는 first 공간을 마련하는데 
...
// 링크 리스트의 값 입력 
	void append(int data) {
		...
		Node3 newNode = new Node3(data); //  데이터 공간 ㅁ 링크 공간 ㅁ // ㅁㅁ  
		 //얘는 어떤 null값을 가리키고 있는 거지? 아니면 아예 새로운 heap 공간에 null이라는 값이 할당이 된 것인가?
		//추가 코딩을 해야함. Node3 p = first ; 처음에
		
		
		if(head==null) {
			head = newNode; // stack  에서 heap 로 바뀌게 된 것? 
