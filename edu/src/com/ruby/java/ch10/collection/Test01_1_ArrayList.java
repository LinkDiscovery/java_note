...
public class Test01_1_ArrayList {
	public static void main(String[] args) {
		...
		arr[0] = s;
		arr[1] = i1;
//		int len = arr[0].length();//Object에 length()가 없다라고 나온다. length()는 String의 메소드 arr[i]는 Object의 메소드
		                           // length( ) > 함수 호출  > 함수 호출 하면 클래스에 가서 바디를 찾는데 , object 클래스에는 length가 없기 때문에 오류 
		                           // explicit type casting 을 활용해야지 오류가 사라진다.
		int len = ((String) arr[0]).length();

		ArrayList lst = new ArrayList
				();//모든 타입의 객체들을 저장할 수 있다 그러나 비추다! - 이유는? // ArrayList의 객체를 만드는데 값을 지정안하면 10개를 할당한다.
		                                //그리고 아무 값도 안넣으면 전부다 Object 객체로 간주 그래서 아래 처럼 정수객체, 스트링 객체 다 호출할 수 있다.
		String s2 = "java";// s2 는 스트링
		Integer i2 = 123; // i2는 정수 객체 , Wrapper 클래스에 의해 박싱이 된것 
		lst.add(s2); // s2는 String 객체 
		lst.add(i2);// i2는 정수 객체네 ? 
...
// 만약에 ArrayList로 String 만 값을 넣고 싶으면 이렇게 사용 
...
//		list.to
		...
		// boolean contains(Object o)              *중요 contains
...
//		                       Collection은 타입  < ?는 wild card처럼 사용 E를 상속하고 밑에꺼만 사.             c는 변수 
		// showData (int[]data
		//           타입   변수
		list.addAll(list2);
		print(13, list);

		// boolean containsAll(Collection<?> c) 모든클래스의 콜렉션은 다된다. 콜렉션의 하위는 다된다.
